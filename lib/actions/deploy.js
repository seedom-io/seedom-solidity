...
module.exports = async (networkName, options) => {
    ...
    const upgrades = await getUpgrades(contractConfig, deployData, options.force);
...
const getUpgrades = async (contractConfig, deployData, forceUpgrade) => {
    ...
    for (let contractName in contractConfig) {
        ...
        if (!forceUpgrade && latestContractDeploy && (latestContractDeploy.hash == contractHash)) {
            continue;
        }
